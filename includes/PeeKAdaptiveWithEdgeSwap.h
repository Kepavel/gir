...
class PeeKAdaptiveWithEdgeSwap : public KSP<GraphType, num_threads, pps> {
   ...
   public:
    ...
    std::vector<Path> compute(const NODE_ID source, const NODE_ID destination, const w_type gap, bool& early_stopped) {
        ...
        // if (k_bound_nodes_num > adaptive_ratio * orig.get_num_nodes()) {
            ...
            result = optyen.compute(source, destination, gap, early_stopped);
            // optyen.print_paths();
            return result;
        // }
        ...
        // start_time = wtime();
        OptYen<DeltaSteppingStatic, GraphType, COMPACT_KSP_NUM_THREADS, COMPACT_KSP_PD> optyen_(compact_g, k, compact_reverse_sssp_t, paths);

#ifdef TECHNIQUE_BOUND
        // float number could cause one less path in some cases, in order to make sure correct totally, now comment it out or solution 2, make the k bound is greater
        if (actual_k == k) {
            optyen_.setKBound(k_bound * 1.01);
        }
#endif

        result = optyen_.compute(compact_source, compact_destination, false, gap, early_stopped);
        // end_time = wtime();
