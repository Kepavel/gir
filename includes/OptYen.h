...
#pragma omp parallel for num_threads(num_threads)
    ...
    std::vector<Path> compute(const NODE_ID source, const NODE_ID destination, bool run_reverse_graph, w_type gap, bool& early_stopped)
    {
        assert(k > 1);
        ...
        if(parallel_ps){
            return compute_pd(source, destination, run_reverse_graph, gap, early_stopped);
        }
...
private:
    ...
    std::vector<Path> compute_pd(const NODE_ID source, const NODE_ID destination, bool run_reverse_graph, const w_type gap, bool& early_stopped)
    {
#if !KSP_PD_L1
        omp_set_nested(1);
#endif
        ...
        for(unsigned int kp = 0; kp < k && !candidates.empty(); kp++)
        {
            ...
            Path p = candidates.get_best();

            if(p.length != std::numeric_limits<w_type>::infinity()){
                paths.push_back(p);
            }          

            // std::cout << "push back" << std::endl;

            // if current path's gap is larger than gap threshold -> early stopping
            if(kp > 0 && kp != k - 1){
                if((p.length - paths[0].length) >= gap){
                    early_stopped = true;
                    return paths;
                }
            }
...
public:
    ...
    std::vector<Path> compute(const NODE_ID source, const NODE_ID destination, w_type gap, bool& early_stopped)
    {
        assert(k > 1);
        ...
        if(parallel_ps){
            // std::cout << "Here is pd" << std::endl;
            return compute_pd(source, destination, gap, early_stopped);
        }
...
private:
    ...
    std::vector<Path> compute_pd(const NODE_ID source, const NODE_ID destination, const w_type gap, bool& early_stopped)
    {
#if !KSP_PD_L1
        omp_set_nested(1);
#endif
