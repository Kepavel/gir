...
public:
        ...
        const char* delta_option )
    {
        ...
        std::cout << "Size of edges: " << sizeof(EDGE_ID) << " Size of nodes: " << sizeof(NODE_ID) << std::endl;
        ...
        // std::cout << "CSR begin: ";
        // for (int i = 0; i <= num_nodes; ++i) std::cout << csr->begin[i] << " ";
        // std::cout << std::endl;

        // std::cout << "CSR adj: ";
        // for (int i = 0; i < num_edges; ++i) std::cout << csr->adj[i] << " ";
        // std::cout << std::endl;

        // std::cout << "CSR value: ";
        // for (int i = 0; i < num_edges; ++i) std::cout << csr->value[i] << " ";
        // std::cout << std::endl;
...
public:
    template<bool directed = false, bool weighted = true, bool add_reverse_graph = false>
    static BasicGraph<directed, weighted> read_graph_bct( int * beg, int * adj, double *value, int num_nodes, int num_edges,
        const char* delta_option){
        ...
        for ( NODE_ID i = 0; i < num_nodes + 1; ++i )
        {
            csr->begin[i] = (EDGE_ID) beg[i];
        }

        // std::cout << "Finished beg" << '\n';

        for ( EDGE_ID i = 0; i < num_edges; ++i )
        {
            // std::cout << i << '\n';
            csr->adj[i] = (NODE_ID) adj[i];
        }

        // std::cout << "Finished csr" << '\n';

        for ( EDGE_ID i = 0; i < num_edges; ++i )
        {
            // std::cout << i << '\n';
            csr->value[i] = (w_type) value[i];
            if ( weighted )
            {
                total_weight += csr->value[i];
                if ( csr->value[i] > heaviest_weight )
        ...
        // std::cout << "Read_BCT, Num Nodes: " << num_nodes << " Num Edges: " << num_edges << std::endl;
